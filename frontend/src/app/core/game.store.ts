...
export interface GameState {
  ...
  doublePointsIfCorrect: number | null;
  loading: boolean;
  error: string | null;
...
export const GameStore = signalStore(
  ...
  withMethods((store, api = inject(GameApiService)) => ({
    ...
    async selectQuestion(questionId: string): Promise<void> {
      ...
      if (!gameId) return;
      patchState(store, { loading: true, currentQuestionId: questionId, activeHint: null, hintPointsIfCorrect: null, doubleActive: false, doublePointsIfCorrect: null, lastResult: null });
      try {
    ...
    async submitAnswer(answer: string): Promise<AnswerResult | null> {
      ...
      try {
        const result = await firstValueFrom(
          api.submitAnswer(gameId, questionId, answer, board.currentPlayerIndex)
        );
    ...
    async use2x(): Promise<void> {
      ...
      try {
        const result = await firstValueFrom(api.use2x(gameId, questionId, board.currentPlayerIndex));
        const updatedBoard = await firstValueFrom(api.getGame(gameId));
        patchState(store, {
          doubleActive: true,
          doublePointsIfCorrect: result.points_if_correct,
          boardState: updatedBoard,
        });
      } catch (err) {
        patchState(store, { error: 'Failed to activate 2x multiplier' });
      }
    },
    ...
    continueToBoard(): void {
      ...
      if (board?.status === 'FINISHED') {
        ...
        patchState(store, { phase: 'board', currentQuestion: null, currentQuestionId: null, lastResult: null, activeHint: null, doubleActive: false, doublePointsIfCorrect: null });
      }
    },
