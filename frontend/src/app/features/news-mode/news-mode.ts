      ...
      <div class="max-w-2xl mx-auto w-full flex flex-col flex-1">
        ...
        <div class="flex items-center justify-between mb-6 pt-2">
          ...
          <div class="text-center">
            <div class="text-accent font-black text-lg">NEWS MODE</div>
            <div class="text-muted-foreground text-xs">{{ lang.t().newsLatestHeadlines }}</div>
          </div>
        ...
        @if (phase() === 'question' && currentQuestion()) {
          <div class="flex flex-col flex-1">
            ...
            @if (currentQuestion()!.fifty_fifty_hint) {
              <p class="text-muted-foreground text-xs mt-3 text-center">
                Hint: it's either "{{ currentQuestion()!.fifty_fifty_hint }}" or the correct answer
              </p>
            }
          </div>
        }
        ...
        @if (phase() === 'result' && lastResult()) {
          <div class="flex flex-col flex-1">
            <div class="rounded-2xl p-6 mb-6 border text-center"
              ...
              @if (currentQuestion()!.source_url) {
                <a [href]="currentQuestion()!.source_url" target="_blank" rel="noopener"
                   class="inline-block text-accent text-xs mt-3 underline">
                  Source →
                </a>
              }
            </div>
            ...
            <div class="bg-card rounded-2xl p-4 mb-6 border border-border flex items-center justify-between">
              ...
              <span class="font-black text-2xl text-accent">{{ correctCount() }} / {{ currentIndex() + 1 }}</span>
            </div>
            ...
            <div class="flex gap-3 mt-auto">
              @if (hasMore()) {
                <button
                  (click)="nextQuestion()"
                  class="flex-1 py-4 rounded-2xl bg-accent text-accent-foreground font-black text-lg hover:bg-accent-light active:scale-95 transition pressable"
                >
                  Next Question
                </button>
              } @else {
          ...
          <div class="flex-1 flex flex-col items-center justify-center">
            ...
            <div class="w-full max-w-xs space-y-3 mb-8">
              ...
              <div class="flex justify-between p-4 bg-card rounded-xl border border-border">
                ...
                <span class="text-accent font-bold">{{ accuracy() }}%</span>
              </div>
            </div>
            <button
              (click)="goHome()"
              class="w-full max-w-xs py-4 rounded-2xl bg-accent text-accent-foreground font-black text-xl hover:bg-accent-light active:scale-95 transition pressable"
            >
              Back to Home
            </button>
          </div>
        ...
        @if (phase() === 'question' && total() === 0) {
          <div class="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <button (click)="goHome()" class="py-3 px-8 rounded-xl bg-accent text-accent-foreground font-bold">Back to Home</button>
          </div>
        }
      </div>
    </div>
