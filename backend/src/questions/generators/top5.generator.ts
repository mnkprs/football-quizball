...
import { GeneratedQuestion, Difficulty, Top5Entry } from '../question.types';
...
export class Top5Generator {
  generate(difficulty: Difficulty, points: number): GeneratedQuestion {
    const pool = SEED_BANK.filter((s) => s.difficulty === difficulty);
