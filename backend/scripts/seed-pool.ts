 ...
 * With min:     npm run pool:seed 1 min 100               (skip categories with ≥100 drawable_unanswered)
 *
 * Automatically verifies and fixes wrong answers / deletes hallucinated questions after seeding.
 *
...
function parseArgs(): { slot?: string; target: number; minDrawable?: number } {
  const args = process.argv.slice(2);

  // Extract "min <N>" pair from anywhere in the args
  let minDrawable: number | undefined;
  const minIdx = args.indexOf('min');
  if (minIdx !== -1 && minIdx + 1 < args.length) {
    const parsed = parseInt(args[minIdx + 1], 10);
    if (!Number.isNaN(parsed) && parsed > 0) minDrawable = parsed;
  ...
  if (args.length === 2) {
    ...
    return { slot, target, minDrawable };
  }
  const targetRaw = args[0] || '1';
  const n = parseInt(targetRaw, 10);
  const target = Number.isNaN(n) ? 1 : Math.min(500, Math.max(1, n));
  return { target, minDrawable };
}

async function runVerify(pool: QuestionPoolService, questionIds: string[]): Promise<void> {
  console.log(
    `\n[seed-pool] Verifying integrity of ${questionIds.length} generated questions (will fix + delete)...`,
  );
  const verifyResult = await pool.verifyPoolIntegrity({ questionIds, apply: true });
  console.log(
  ...
  if (verifyResult.fixed > 0 || verifyResult.deleted > 0) {
    console.log(`  Applied: ${verifyResult.fixed} fixed, ${verifyResult.deleted} deleted.`);
  }
}

async function main() {
  const { slot, target, minDrawable } = parseArgs();
  ...
  if (slot) {
    ...
    if (result.questions?.length) {
      ...
      await runVerify(pool, result.questions);
    }
  } else {
    console.log(`[seed-pool] All categories, target passes: ${target}${minDrawable ? `, min drawable: ${minDrawable}` : ''}`);
    const { results, sessionId, questionIds } = await pool.seedPool(target, true, { minDrawable });
    ...
    if (questionIds.length > 0) {
      ...
      await runVerify(pool, questionIds);
    }
  }
  await app.close();
}
